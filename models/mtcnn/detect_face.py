...
def detect_face(imgs, minsize, pnet, rnet, onet, threshold, factor, device):
    if isinstance(imgs, (np.ndarray, torch.Tensor)):
        ...
        # imgs = np.stack([np.uint8(img) for img in imgs]) # commented out to work directly with torch images
        # imgs = torch.as_tensor(imgs.copy(), device=device) # commented out to work directly with torch images

    model_dtype = next(pnet.parameters()).dtype
    # imgs = imgs.permute(0, 3, 1, 2).type(model_dtype)
    imgs = imgs.type(model_dtype)  # editied to remove redundancy
...
def crop_resize(img, box, image_size):
    if isinstance(img, np.ndarray):
        ...
        # Determine the difference and how much to add to each side
        if width > height:
            diff = width - height
            box[1] -= diff // 2
            box[3] += diff // 2
...
def extract_face(img, box, image_size=160, margin=0, save_path=None):
    ...
    face = face.permute(2, 0, 1)  # edited to work directly with torch images
