        ...
        if ("double_block", i) in blocks_replace:
            ...
            def block_wrap(args):
                ...
                out["img"], out["txt"] = block(
                    img=args["img"], txt=args["txt"], vec=args["vec"], pe=args["pe"], attn_mask=args.get("attn_mask")
                )
                return out
    ...
    def apply_pulid_flux(
        ...
        if prior_image is not None:
            prior_image = resize_with_pad(
                prior_image.to(image.device, dtype=image.dtype), target_size=(image.shape[1], image.shape[2])
            )
        ...
        if not cond:
            raise ValueError("PuLID error: No faces detected in any of the given images.")
        ...
        if fusion == "mean":
            ...
            if cond.shape[0] > 1:
                ...
                regular_weight = torch.linspace(fusion_weight_min, fusion_weight_max, norm.shape[0]).to(
                    device, dtype=dtype
                )
